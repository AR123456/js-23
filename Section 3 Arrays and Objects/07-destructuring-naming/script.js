// Setting object properties with the same name as a variable
const firstName = "John";
const lastName = "Doe";
const age = 30;

// can be writen long hand but when the variable defining the value is the same as the key can
// const person = {
//   firstName: firstName,
//   lastName: lastName,
//   age: age,
// };
//just pass in one time
const person = {
  firstName,
  lastName,
  age,
};

console.log(person.age);

// Destructuring object properties

const todo = {
  id: 1,
  title: "Take out trash",
  user: {
    name: "John",
  },
};
const id = todo.id;
console.log(id);
// destructure properties from the object
//  syntax const { a, b } = obj;

const {
  id: todoId, // rename id to todoId
  title,
  user: { name }, // destructuring multiple levels
} = todo;

console.log(todo); // the whole todo object
console.log(todoId);

// Destructuring arrays & using the rest/spread operator
const numbers = [23, 67, 33, 49, 52];

const [first, second, ...rest] = numbers;

console.log(first, second, rest);
